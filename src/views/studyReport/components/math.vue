...
function initChart() {
  let chartDom = document.getElementById("mathChartContainer");
...
function intoResult(id: number | undefined) {
  router.push({ path: "/markingResult", query: { id } });
}
setTimeout(() => {
  initChart();
}, 1000);
...
<template>
  <div class="container">
    <div class="bg-white h-15vh w-full my-5 border-rd-14px p-4 box-border">
      <van-row>
        <van-col span="12">{{ props.data?.name || "数学" }}</van-col>
        <van-col span="12" class="cursor-pointer text-gray-400">
          <span @click="intoResult(props.data?.examId)">查看阅卷结果</span>
        </van-col>
      </van-row>
      <van-row>
...
<style>
.van-tabs__content #mathChartContainer {
  width: 90vw;
  height: 40vh;
}
</style>
