...
function initChart() {
  let chartDom = document.getElementById("englishChartContainer");
...
function loadTrend() {
  getTrend(pagination as Paging<{ subjectId: string }>).then((res) => {
    console.log(res);
...
<template>
  <div class="container">
    <div class="bg-white h-15vh w-full my-5 border-rd-14px p-4 box-border">
      <van-row>
        <van-col span="24">{{ props.data?.name || "英语" }}</van-col>
      </van-row>
      <van-row>
        <van-col span="12" style="line-height: 3.5rem"
          ><span class="text-size-13">{{ props.data?.score || "-" }}</span
          ><span>分</span></van-col
        >
...
<style scoped>
.van-tabs__content #englishChartContainer {
  width: 90vw;
  height: 40vh;
}
</style>
